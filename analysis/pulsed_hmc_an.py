"""
This module provides analysis of pulsed measurements with AWG and HMC microwave source.
...
import measurements.pulsed_hmc as pm_hmc
...
class PulsedFit(HasTraits, GetSetItemsMixin):
    ...
    measurement = Instance(pm_hmc.PulsedAWG, factory=pm_hmc.PulsedAWG)
...
class PulsedAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_rabi_measurement(self, info):
        ...
        info.object.measurement = pm_hmc.RabiAWG()
...
class PulsedAnalyzer(HasTraits, GetSetItemsMixin):
    ...
    traits_view = View(HGroup(VGroup(Item(name='measurement', style='custom', show_label=False),
                       ...
                       title='Pulsed Analyzer for Rabi HMC+AWG', buttons=[], resizable=True, handler=PulsedAnalyzerHandler)
...
class SensingPulsedAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_Hahn_measurement(self, info):        
        info.object.measurement = pm_hmc.Hahn()    
        
    def new_T1_measurement(self, info):        
        info.object.measurement = pm_hmc.T1()     
...
class DoubleSensingPulsedAnalyzer(HasTraits, GetSetItemsMixin):
    ...
    traits_view = View(HGroup(VGroup(Item(name='measurement', style='custom', show_label=False),
                       ...
                       menubar=MenuBar(Menu(Action(action='save', name='Save (.pyd or .pys)'),
                                              ...
                                              Action(action='_on_close', name='Quit'),
                                              name='File'),
                                       Menu(Action(action='new_Hahn_measurement', name='Hahn'),
                       ...
                       title='Analyzer for AWG + HMC measurements', buttons=[], resizable=True, handler=SensingPulsedAnalyzerHandler)
...
class RabiFit_phase(PulsedFit):
    ...
    measurement = Instance(pm_hmc.PulsedAWG, factory=pm_hmc.RabiAWG)
