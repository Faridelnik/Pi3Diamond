...
import measurements.pulsed_hmc as mp_hmc
...
class PulsedHMCFit(HasTraits, GetSetItemsMixin):
    ...
    measurement = Instance(mp_hmc.Pulsed, factory=mp_hmc.Pulsed)
    ...
    def __init__(self):
        super(PulsedHMCFit, self).__init__()
        self.on_trait_change(self.update_plot_spin_state, 'spin_state', dispatch='ui')
...
class RabiFitHMC(PulsedHMCFit):

    """Provides fits and plots for a Rabi    measurement."""

    measurement = Instance(mp_hmc.Pulsed, factory=mp_hmc.Rabi)
    ...
    def __init__(self):
        super(RabiFitHMC, self).__init__()
        self.on_trait_change(self.update_fit, 'spin_state', dispatch='ui')
        self.on_trait_change(self.update_plot_tau, 'measurement.tau', dispatch='ui')
        self.on_trait_change(self.update_plot_fit, 'fit_result', dispatch='ui')
    ...
    get_set_items = PulsedHMCFit.get_set_items + ['fit_result', 'contrast', 'period', 't_pi2', 't_pi', 't_3pi2', 'text']
    
class RabiFitHMC_phase(PulsedHMCFit):
    ...
    def __init__(self):
        super(RabiFitHMC_phase, self).__init__()
        self.on_trait_change(self.update_fit, 'spin_state', dispatch='ui')
        self.on_trait_change(self.update_plot_tau, 'measurement.tau', dispatch='ui')
        self.on_trait_change(self.update_plot_fit, 'fit_result', dispatch='ui')
    ...
    get_set_items = PulsedHMCFit.get_set_items + ['fit_result', 'contrast', 'period', 'phase','t_pi2', 't_pi', 't_3pi2', 'text']        
...
class DoublePulsedFit(HasTraits, GetSetItemsMixin):
    ...
    def update_spin_state(self):
        if self.measurement is None:
            return
        y1, profile, flank = spin_state(c=self.measurement.count_data[0::2],
                                        dt=self.measurement.bin_width,
                                        T=self.integration_width,
                                        t0=self.position_signal,
                                        t1=self.position_normalize,)
        y2, profile, flank = spin_state(c=self.measurement.count_data[1::2],
                                        dt=self.measurement.bin_width,
                                        T=self.integration_width,
                                        t0=self.position_signal,
                                        t1=self.position_normalize,)                               
        ...
        if self.T1:
            self.free_evolution_time=(self.x_tau+self.measurement.t_pi)
        else:
            self.free_evolution_time=(2*self.x_tau+self.measurement.t_pi)
...
class PulsedAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_fid_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp.FID()    
        
    def new_hahn_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp.Hahn()    
        
    def new_rabi_hmc_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp_hmc.Rabi()
    
    def new_fid_hmc_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp_hmc.FID()    
        
    def new_hahn_hmc_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp.Hahn_hmc()        
...
class PulsedHMCAnalyzerHandler(GetSetItemsHandler):
    ...
    def new_hahn_hmc_measurement(self, info):
        """info.object.measurement = mp.Rabi()"""
        info.object.measurement = mp_hmc.Hahn()        
    
    # new fits 

    def new_pulsed_fit(self, info):
        info.object.fit = PulsedHMCFit()
    def new_rabi_fit(self, info):
        info.object.fit = RabiFitHMC()
    def new_rabiphase_fit(self, info):
        info.object.fit = RabiFitHMC_phase()    
    
class PulsedHMCAnalyzer(HasTraits, GetSetItemsMixin):
    ...
    measurement = Instance(mp_hmc.Pulsed, factory=mp_hmc.Pulsed)
    fit = Instance(PulsedHMCFit, factory=PulsedHMCFit)
    ...
    def __init__(self):
        super(PulsedHMCAnalyzer, self).__init__()
        self.on_trait_change(self.refresh_matrix_axis, 'measurement.time_bins,measurement.n_laser', dispatch='ui')
        self.on_trait_change(self.refresh_matrix, 'measurement.count_data', dispatch='ui')
        self.on_trait_change(self.refresh_pulse, 'fit.pulse', dispatch='ui')
        self.on_trait_change(self.refresh_time_bins, 'measurement.time_bins', dispatch='ui')
    ...
    traits_view = View(HGroup(VGroup(Item(name='measurement', style='custom', show_label=False),
                       ...
                       menubar=MenuBar(Menu(Action(action='save', name='Save (.pyd or .pys)'),
                                  ...
                                  #TODO            Action(action='save_all', name='Save Line Plot (.png) + .pys'),
                                              Action(action='_on_close', name='Quit'),
                                              name='File'),
                                         Menu(Action(action='new_rabi_hmc_measurement', name='Rabi_HMC'),
                                              Action(action='new_fid_hmc_measurement', name='FID_HMC'),
                                              Action(action='new_hahn_hmc_measurement', name='Hahn_HMC'),
                                              #Action(action='new_opticalrabi_measurement', name='OpticalRabi'),
                                              name='Measurement'),
                                         Menu(Action(action='new_pulsed_fit', name='Pulsed'),
                                              Action(action='new_rabi_fit', name='Rabi'),
                       ...
                       title='PulsedHMCAnalyzer', buttons=[], resizable=True, handler=PulsedHMCAnalyzerHandler)
...
class DoubleSensingPulsedAnalyzer(HasTraits, GetSetItemsMixin):
    ...
    traits_view = View(HGroup(VGroup(Item(name='measurement', style='custom', show_label=False),
                       ...
                       title='T1, T2 without AWG', buttons=[], resizable=True, handler=SensingPulsedAnalyzerHandler)
