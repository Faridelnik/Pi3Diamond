...
from hardware.nidaq import AOBoard

class Confocal( ManagedJob, GetSetItemsMixin ):
    # laser power
    slider = Range(value=-7.65, low=-10.0, high=10.0, desc="AOM Amplitude") # ~120 uW, nonlinear scale. -7 is already ~450uW
    ...
    seconds_per_point = Range(low=1e-6, high=10, value=5e-5, desc='Seconds per point [s]', label='Seconds per point [s]', mode='text', auto_set=False, enter_set=True)
    scan_speed = Range(low=0.0, high=400.0, value=200.0, desc='Scan speed Hz', label='scan speed Hz', auto_set=False, enter_set=True)
    ...
    def __init__(self,scanner):
        ...
        self.on_trait_change(self._calculate_speed, 'seconds_per_point, resolution', dispatch='ui')

    # scanner position
    
    def _calculate_speed(self):
        self.scan_speed=1.0/(self.seconds_per_point*self.resolution)
    ...
    def _slider_changed(self, new):
        self.ao.Write(numpy.array((new), dtype=numpy.float64) )
