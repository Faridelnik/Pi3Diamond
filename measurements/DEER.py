...
class PulsedDEER(ManagedJob, GetSetItemsMixin):
    ...
    time_Hahn = Range(low=1., high=5e6, value=5000., desc='time Hahn [ns]', label='time Hahn [ns]', mode='text', auto_set=False, enter_set=True) # for NV center
    ...
    freq_HMC = Range(low=10e+6, high=20e9, value=4e9, desc='HMC freq [Hz]', label='HMC freq [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    power_HMC = Range(low= -35., high=25., value= -2, desc='HMC Power [dBm]', label='HMC power [dBm]', mode='text', auto_set=False, enter_set=True)
    ...
    tau = Array(value=np.array((0., 1.))) # time for electron spins
    
    def __init__(self):
        super(PulsedDEER, self).__init__()
    ...
    def _run(self):
        ...
        try: # try to run the acquisition from start_up to shut_down
            ...
            MW_HMC.setFrequency(self.freq_HMC)
            MW_HMC.setPower(self.power_HMC)
    ...
    get_set_items = ['__doc__', 'record_length','laser','wait', 'bin_width', 'n_bins', 'time_bins', 'n_laser', 'sequence', 'count_data', 'run_time','tau_begin', 'tau_end', 'tau_delta', 'tau', 'time_Hahn', 'power', 'power_HMC']
    ...
    traits_view = View(VGroup(HGroup(Item('submit_button', show_label=False),
                              ...
                              HGroup(Item('freq', width=40),
                                     Item('power', width=40),
                                     Item('freq_HMC', width=40),
                                     Item('power_HMC', width=40),
                                     ),       
                       ...
                       title='Pulsed DEER Measurement',
                       )
                 
class Electron_Rabi( PulsedDEER ):
    ...
    def __init__(self):
        super(Electron_Rabi, self).__init__()
    ...
    def generate_sequence(self):
        ...
        for t in tau:
            ...
            sequence.append(  (['hmc_trigger'   ],   t            )  )
            sequence.append(  ([       ],            time_Hahn-t  )  )
            sequence.append(  (['mw'   ],            t_pi2        )  )
            sequence.append(  (['laser', 'trigger'], laser        )  )
            sequence.append(  ([       ],            wait         )  )
        return sequence
        
    get_set_items = PulsedDEER.get_set_items + ['t_pi2','t_pi']
...
class DEER( PulsedDEER ):
    ...
    t_el_pi = Range(low=1., high=100000., value=1000., desc='length of pi pulse [ns]', label='pi [ns]', mode='text', auto_set=False, enter_set=True)
