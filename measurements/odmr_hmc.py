...
class ODMR_HMC(ManagedJob, GetSetItemsMixin):
    """Provides ODMR measurements."""
    """Freq conversion is used here"""
    ...
    frequency_LO = Range(low=0.8e9, high=20e9, value=4e9, desc='Carrier Frequency [Hz]', label='LO_Freq[Hz, < Begin]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    power_LO = Range(low= -35., high=14., value= 6, desc='Carrier Power [dBm]', label='LO_Power [dBm]', mode='text', auto_set=False, enter_set=True)
    power = Range(low= -100., high=25., value= -8, desc='Power [dBm]', label='Power [dBm]', mode='text', auto_set=False, enter_set=True)
    frequency_begin = Range(low=0.8e9, high=20e9, value=2.85e9, desc='Start Frequency [Hz]', label='Begin [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    frequency_end = Range(low=0.8e9, high=20e9, value=2.90e9, desc='Stop Frequency [Hz]', label='End [Hz]', editor=TextEditor(auto_set=False, enter_set=True, evaluate=float, format_str='%e'))
    ...
    central_dip_position = Float(value=0.0, desc='Centr freq [GHz]', label='Centr freq [GHz]', editor=TextEditor(auto_set=False, enter_set=False, evaluate=float, format_str='%e'))
    magnetic_field = Float(value=0.0, desc='Magnetic field [G]', label='Magnetic field [G]', editor=TextEditor(auto_set=False, enter_set=False, evaluate=float, format_str='%e'))
    crossing = Bool(False, label='crossing')
    

    def __init__(self):
        super(ODMR_HMC, self).__init__()
        ...
        self.on_trait_change(self._update_magnetic_field, 'counts', dispatch='ui')
    ...
    def _run(self):
        ...
        try:
            ...
            if self.pulsed:
                ha.PulseGenerator().Sequence([(['laser'],self.laser), ([], self.wait), (['mw'], self.t_pi), ([],15)])
            else:
            ...
            if self.pulsed:
                ha.Microwave_HMC().setFrequency(self.frequency_LO)
                ha.Microwave_HMC().setPower(self.power_LO)
                
                ha.Microwave().setPower(self.power_p)
                ha.Microwave().initSweep(self.frequency-self.frequency_LO, self.power_p * np.ones(self.frequency.shape))
            else:
                ha.Microwave_HMC().setFrequency(self.frequency_LO)
                ha.Microwave_HMC().setPower(self.power_LO)
            ...
            if self.run_time < self.stop_time:
                ...
                self.state = 'done'
            if self.pulsed:
                ha.Microwave().setOutput(None, self.frequency_begin_p--self.frequency_LO)
                ha.Microwave_HMC().Off()
            else:
                ha.Microwave().setOutput(None, self.frequency_begin-self.frequency_LO)
                ha.Microwave_HMC().Off()
    ...
    def _update_magnetic_field(self):
        if len(self.fit_frequencies)>0:
            if self.crossing:
                self.magnetic_field = (2870+self.fit_frequencies[0]*1e-6)/2.8
            else:
                self.magnetic_field = (2870-self.fit_frequencies[0]*1e-6)/2.8
            self.central_dip_position = self.fit_frequencies.sum()/len(self.fit_frequencies)
    ...
    traits_view = View(VGroup(HGroup(Item('submit_button', show_label=False),
                               ...
                               HGroup(Item('power_LO', width= -40, enabled_when='state != "run"'),
                                     Item('frequency_LO', width= -80, enabled_when='state != "run"'),
                                    
                                     ),      
                               Group( HGroup(Item('pulsed', enabled_when='state != "run"'),
                       ...
                       menubar=MenuBar(Menu(Action(action='saveLinePlot', name='SaveLinePlot (.png)'),
                                              ...
                                              name='File')),
                       title='ODMR_HMC', width=900, height=800, buttons=[], resizable=True, handler=ODMRHandler
                       )

    get_set_items = ['frequency', 'counts', 'counts_matrix',
                     'fit_parameters', 'fit_contrast', 'fit_line_width', 'fit_frequencies',
                     'perform_fit', 'run_time','power_LO','frequency_LO',
