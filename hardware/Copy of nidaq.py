...
class GalvoPiezo( MultiBoard ):

    def __init__(self, CounterIn, CounterOut, TickSource, AOChannels,
                 x_range, y_range, z_range, v_galvo_range=(-10.,10.),v_piezo_range=(0.,10.),
                 invert_x=False, invert_y=False, invert_z=False, swap_xy=False, TriggerChannels=None):
        MultiBoard.__init__(self, CounterIn=CounterIn,
                                  CounterOut=CounterOut,
                                  TickSource=TickSource,
        ...
        self.yRange = y_range
        self.zRange = z_range
        self.vgRange = v_galvo_range
        self.vpRange = v_piezo_range
        self.x = 0.0
        self.y = 0.0
        self.z = 0.0
        self.invert_x = invert_x
    ...
    def PosToVolt(self, r):
        ...
        if self.invert_y:
            ...
            vy = v0+(r[1]-y[0])/(y[1]-y[0])*dv            
        if self.invert_z:
            vz = vp0+(z[1]-r[2])/(z[1]-z[0])*dvp            
        else:
            vz = vp0+(r[2]-z[0])/(z[1]-z[0])*dvp
        if self.swap_xy:
            vt = vx
            vx = vy
            vy = vt            
