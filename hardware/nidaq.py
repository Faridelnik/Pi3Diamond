...
class AOBoard():
    ...
    def __init__(self, AOChannels, v_range=(-10.0, 10.0)):
        self._AODevice = AOChannels
...
class Scanner( MultiBoard ):
    ...
    def PosToVolt(self, r):
        ...
        if self.invert_y:
            vy = v0+0.4+(y[1]-r[1])/(y[1]-y[0])*(dv-0.4)            
        else:
            vy = v0+0.4+(r[1]-y[0])/(y[1]-y[0])*(dv-0.4) 
        if self.invert_z:
...
class SST_PulseTrainCounter:

    def __init__(self, photon_source,  function_counter_in,  gate_in_channel):
        self._photon_source = photon_source
        self._function_counter_in = function_counter_in
        self._gate_in_channel = gate_in_channel
    ...
    def start_gated_counting(self):
        ...
        self.gated_count_task = ctypes.c_uint32() #this task will count photons with binning defined by pulse task
        ...
        CHK(  dll.DAQmxSetCIPulseWidthTerm( self.gated_count_task, self._function_counter_in, self._gate_in_channel)  )
        ...
        CHK(  dll.DAQmxCfgImplicitTiming( self.gated_count_task,
                                          DAQmx_Val_ContSamps,
                                          ctypes.c_ulonglong(self.buffer_length))  )
    ...
    def read_gated_counts(self, SampleLength, timeout=600):
        """Returns latest count samples aquired by gated photon counting."""
        self._gated_count_data = numpy.empty((SampleLength,), dtype=numpy.uint32) # count data will be written here
        ...
        CHK( dll.DAQmxReadCounterU32(self.gated_count_task   #read from this task
                                     , ctypes.c_int32(SampleLength)    #read number samples
                                     ...
                                     , ctypes.c_uint32(SampleLength)   #length of array to write into
    ...
    def configure(self, MaxCounts=1e7, RWTimeout=100):

        if hasattr(self, 'gated_count_task'):
            self.clear()
        ...
        CHK(  dll.DAQmxCreateCIPulseWidthChan( self.gated_count_task,    #add to this task
                                               ...
                                               '',  #name
                                               ctypes.c_double(0),  #expected minimum value
                                               ctypes.c_double(MaxCounts),    #expected maximum value
                                               DAQmx_Val_Ticks,     #units of width measurement, here photon ticks
                                               DAQmx_Val_Rising, '')   ) #start pulse width measurement on rising edge
        ...
        self.n_read_samples = ctypes.c_int32()
    ...
    def start_gated_counting(self):
        ...
        #        return -1
        self.function_state = 'gated'
        self.gated_count_task = ctypes.c_ulong() #this task will count photons with binning defined by pulse task
        CHK( dll.DAQmxCreateTask('', ctypes.byref(self.gated_count_task)) )   #create task
        #set up pulse width measurement in photon ticks, i.e. the width of each pulse generated by pulse_out_task is measured in photon ticks.
        CHK(  dll.DAQmxCreateCIPulseWidthChan( self.gated_count_task,    #add to this task
                                               self._function_counter_in,  #use this counter
    ...
    def read_gated_counts(self,SampleLength):
        """Returns latest count samples aquired by gated photon counting."""
        self._gated_count_data = numpy.empty((SampleLength,), dtype=numpy.uint32) # count data will be written here
        n_read_samples = ctypes.c_int32() #number of samples which were read will be stored here
        CHK( dll.DAQmxReadCounterU32(self.gated_count_task   #read from this task
                                     , ctypes.c_int32(SampleLength)    #read number samples
                                     , ctypes.c_double(self._RWTimeout)     # Timeout of read. Since nidaq waits for all samples to be aquired, make sure this is long enough.
                                     , self._gated_count_data.ctypes.data_as(c_uint32_p) #write into this array
                                     , ctypes.c_uint32(SampleLength)   #length of array to write into
                                     , ctypes.byref(self.n_read_samples), None) ) #number of samples which were read
        return self._gated_count_data[:]
...
class PulseTrainCounter:
    ...
    def configure(self, SampleLength, SecondsPerPoint, DutyCycle=0.8, MaxCounts=1e7, RWTimeout=1.0):
...
class PulseTrainCounterExternal:
    """Outputs pulsed train and performs gated count."""

    def __init__(self, CounterIn, GateSignal, TickSource):
        self._CounterIn = CounterIn
        self._GateSignal = GateSignal
        self._TickSource = TickSource

    def configure(self, SampleLength, SecondsPerPoint, MaxCounts=1e7, RWTimeout=1.0):

        if hasattr(self, '_CITask') or hasattr(self, '_COTask'):
            self.clear()
        
        # 0.025ms is the interval between two frequency points, which is determined by the HMC microwave source
        f = 1. / (SecondsPerPoint + 0.025)
        DutyCycle = SecondsPerPoint * f
        ...
        CHK(  dll.DAQmxCreateCIPulseWidthChan( self._CITask,
                                               ...
                                               DAQmx_Val_Ticks, DAQmx_Val_Rising, '')   )
        # set gate signal
        CHK(  dll.DAQmxSetCIPulseWidthTerm( self._CITask, self._CounterIn, self._GateSignal )  )
        # set source for counting
        CHK(  dll.DAQmxSetCICtrTimebaseSrc( self._CITask, self._CounterIn, self._TickSource )  )
