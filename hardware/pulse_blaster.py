...
map = {'laser':2, 'pumping':2, 'mw':1, 'microwave':1, 'trigger':3, 'SequenceTrigger':0, 'awgTrigger':0, 'sst':4, 'csd':5, 'hmc_trigger':6}
...
def High(channels):
    ...
    logging.getLogger().debug(str(channels))
    dll.pb_close()
    chk(dll.pb_init())
...
def Sequence(sequence, loop=True):
    ...
    logging.getLogger().debug(str(sequence))
